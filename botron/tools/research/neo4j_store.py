"""Neo4j persistence backend for :mod:`botron.research.graph`.
...
``botron.research._state``:

- ``BOTRON_KG_BACKEND=neo4j``
- ``BOTRON_NEO4J_URI``
- ``BOTRON_NEO4J_USER``
- ``BOTRON_NEO4J_PASSWORD``
- ``BOTRON_NEO4J_DATABASE`` (optional, default: ``neo4j``)
...
from botron.tools.research.graph import Edge, EdgeKind, KnowledgeGraph, Node, NodeKind
...
class Neo4jConfig:
    ...
    def from_env(cls) -> Neo4jConfig:
        uri = os.environ.get("BOTRON_NEO4J_URI", "").strip()
        user = os.environ.get("BOTRON_NEO4J_USER", "").strip()
        password = os.environ.get("BOTRON_NEO4J_PASSWORD", "").strip()
        database = os.environ.get("BOTRON_NEO4J_DATABASE", "neo4j").strip() or "neo4j"

        missing: list[str] = []
        if not uri:
            missing.append("BOTRON_NEO4J_URI")
        if not user:
            missing.append("BOTRON_NEO4J_USER")
        if not password:
            missing.append("BOTRON_NEO4J_PASSWORD")
