...
cache under ``~/.botron/references/<slug>/`` (override via
``BOTRON_REFERENCES_ROOT``) so agents can grep through the full
...
from botron.tools.references.catalog import REFERENCES, ReferenceEntry


def _default_cache_root() -> Path:
    override = os.environ.get("BOTRON_REFERENCES_ROOT")
    if override:
        return Path(override)
    return Path.home() / ".botron" / "references"
