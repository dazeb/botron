...
Model selection is controlled by the profile axis:

  Profile  — which model tier to use (cost/performance tradeoff)

Profiles:
  eco  — Balanced multi-provider ensemble (production engagements)
  max  — Maximum performance (high-value targets)
  test — Cheapest possible (development and CI)

Usage:
  mapping = LLMModelMapping.from_profile("eco")

  BOTRON_MODEL_PROFILE=eco   (env var, default: eco)
...
class LLMModelMapping(BaseModel):
    ...
    def from_profile(cls, profile: ModelProfile | str) -> LLMModelMapping:
        ...
        if profile == ModelProfile.TEST:
            return cls(
                botron=ModelAssignment(primary=HAIKU, temperature=0.4),
