...
2. **Tool prompt co-location** — tool prompts live with their tool code (e.g.,
   botron/tools/bash/prompt.py) and are injected here with role-specific variations.
...
def _get_tool_prompt(tool_name: str, role: str | None = None) -> str:
    ...
    """
    if tool_name == "bash":
        from botron.tools.bash.prompt import get_bash_prompt
...
# The orchestrator (botron) and soundwave don't do direct tool execution,
