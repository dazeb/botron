...
from botron.core.subagent_streaming import StreamingRunnable
from botron.llm import LLMFactory
from botron.middleware import EngagementContextMiddleware, OPPLANMiddleware
...
def create_decepticon_agent():
    ...
    factory = LLMFactory()
    llm = factory.get_model("botron")
    fallback_models = factory.get_fallback_models("botron")
    ...
    system_prompt = load_prompt("botron", shared=["bash"])
    ...
    from botron.agents.ad_operator import create_ad_operator_agent
    from botron.agents.analyst import create_analyst_agent
    from botron.agents.cloud_hunter import create_cloud_hunter_agent
    from botron.agents.contract_auditor import create_contract_auditor_agent
    ...
    from botron.agents.reverser import create_reverser_agent
    ...
    # botron delegation. Document edits while docs already exist are
    # handled by botron's FilesystemMiddleware directly.
    subagents = [
        CompiledSubAgent(
            name="recon",
            description=(
    ...
    middleware = [
        ...
        DecepticonSkillsMiddleware(
            backend=backend, sources=["/skills/botron/", "/skills/shared/"]
        ),
