...
Toggle via env vars (read once at import / first ``configure_logging`` call):
    ...
    BOTRON_LOG_LEVEL    DEBUG | INFO (default) | WARNING | ERROR
    BOTRON_LOG_FORMAT   text (default) | json
...
def configure_logging(
    ...
    """Configure the root ``botron`` logger.

    Idempotent: calling twice replaces the prior handler so tests can
    reconfigure freely. Honors env vars when args are omitted.
    """
    global _CONFIGURED
    level = level or os.getenv("BOTRON_LOG_LEVEL", "INFO")
    fmt = (fmt or os.getenv("BOTRON_LOG_FORMAT", "text")).lower()
...
def get_logger(name: str) -> logging.Logger:
    ...
    if not _CONFIGURED:
        configure_logging()
    return logging.getLogger(f"botron.{name}")
