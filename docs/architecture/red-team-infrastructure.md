...
graph TB
    subgraph HostMachine["Host Machine"]
        ...
        subgraph DecepticonNet["botron-net (인프라 네트워크)"]
...
graph LR
    ...
    style botron-net fill:#1a1a2e,stroke:#e94560,color:#fff
    style sandbox-net fill:#0f3460,stroke:#e94560,color:#fff
```

- **botron-net**: 인프라 서비스 전용. 외부 포트는 `127.0.0.1`에만 바인딩
...
| 항목 | 값 |
|------|-----|
| 이미지 | `botron-sandbox` (Kali rolling) |
| 메모리 | 4GB |
| CPU | 2코어 |
| PID 제한 | 1024 |
| 네트워크 | `sandbox-net` |
...
| 항목 | 값 |
|------|-----|
| 이미지 | `botron-c2-sliver` (Kali rolling) |
| Profile | `c2-sliver` |
| 메모리 | 2GB |
| CPU | 1코어 |
| PID 제한 | 512 |
...
| 서비스 | 포트 | 바인딩 | 네트워크 |
|--------|------|--------|----------|
| LangGraph API | 2024 | 127.0.0.1 | botron-net |
| LiteLLM Proxy | 4000 | 127.0.0.1 | botron-net |
| PostgreSQL | 5432 | 127.0.0.1 | botron-net |
